    ...
    <section className="section-padding relative overflow-hidden">
      ...
      <div className="container-custom relative z-10">
        ...
        >
          ...
          <motion.p variants={fadeInUp} className="text-white/80 text-lg max-w-2xl mx-auto mb-10">
            Jangan biarkan tugas menghalangi aktivitas pentingmu. Chat kami sekarang dan dapatkan
            konsultasi gratis!
          </motion.p>
          ...
          >
            {/* Payment Methods */}
            <div className="flex flex-col sm:flex-row items-center gap-3 bg-white/10 backdrop-blur-sm rounded-2xl sm:rounded-full px-4 sm:px-6 py-3">
              <span className="text-white/60 text-sm">Payment:</span>
              <div className="flex flex-wrap justify-center items-center gap-2">
                {["BCA", "Mandiri", "GoPay", "OVO", "DANA"].map((method) => (
                  <span
                    key={method}
                    className="text-white text-xs font-medium px-3 py-1.5 bg-white/20 rounded-lg"
                  >
                    {method}
                  </span>
                ))}
